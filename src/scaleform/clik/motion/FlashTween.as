 ...
 *
 * Usage:
 * var tween1:FlashTween = new FlashTween(1000, myObject, {x:250, y:250, alpha:0}, {paused:false, ease:Strong.easeOut, onComplete:handleTweenComplete, loop:true});
 */

/**************************************************************************

 Filename    :   FlashTween.as
...
package scaleform.clik.motion
{
	...
	internal class FlashTween implements ITween
	{

		public static var autoDispose:Boolean = false;

		protected static var ticker:Shape = new Shape();
		protected static var workingMatrix:Matrix = new Matrix();
		ticker.addEventListener(Event.ENTER_FRAME, FlashTween.tick, false, 0, true);

		protected static var firstTween:FlashTween;
		...
		protected static function tick(evt:Event):void {
			...
			lastTime = t;
			var tween:FlashTween = firstTween;
			while (tween) {
				var nextTween:FlashTween = tween.next;
		...
		protected static function removeTween(tween:FlashTween):void {
		...
		public var nextTween:FlashTween;             // Used if you want to chain another tween. The chained tween will be set to pause=false when the current Tween finishes.
		...
		protected var next:FlashTween; // linked list
		protected var prev:FlashTween; // linked list
		 ...
		 */
		public function FlashTween(parent:Tween, duration:Number, target:Object = null, props:Object = null, quickSet:Object = null) {
			this.parent = parent;
		...
		protected function init():void {
			...
			for (var name:String in props) {
				if (fastTransform && transform && propsDO[name]) {
					useMatrix = true;
					continue;
				}
				var prop:Prop = constructProp(name);
				prop.delta = props[name] - (prop.start = _target[name]);
			}
			if (useMatrix) {
				startMatrix = new Matrix(targetDO.scaleX, targetDO.rotation * degToRad,
						targetDO.alpha,
						targetDO.scaleY,
						targetDO.x,
						targetDO.y);
		...
		protected function updatePosition(value:Number):void {
			...
			while (prop) {
				_target[prop.name] = prop.start + prop.delta * ratio;
				prop = prop.next;
			}

			if (onChange != null) { onChange(parent); }
			if (complete) {
				var doDispose:Boolean = false;
				if (loop) { reset(); }
				else { doDispose = true; }
				if (nextTween) { nextTween.paused = false; }
				if (onComplete != null) { onComplete(parent); }
		...
		public function doAutoDispose():void
		{
			...
			if (targetDO) targetDO = null;
			if (startMatrix) startMatrix = null;
			if (deltaMatrix) deltaMatrix = null;
