...
package scaleform.clik.utils {
	...
	public class Constraints extends EventDispatcher {
		...
		public static const CENTER_H_RELATIVE:uint = 64;
		public static const CENTER_V_RELATIVE:uint = 128;
		...
		public function update(width:Number, height:Number):void {
			...
			for (var n:String in elements) {
				...
				if ((edges & Constraints.CENTER_H_RELATIVE) > 0) {
					clip.x = int((width + element.left - element.right) * xAdjust / 2 - clip.width / 2);
				}
				
				if ((edges & Constraints.CENTER_V_RELATIVE) > 0) {
					clip.y = int((height + element.top - element.bottom) * yAdjust / 2 - clip.height / 2);
				}
