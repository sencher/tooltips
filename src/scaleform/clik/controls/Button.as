...
public class Button extends UIComponent {
    ...
    public function Button() {
        super();
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.greencw(this, DebugUtils.set("Button.constructor _currentFrame", _currentFrame, currentFrameLabel, name));
        _currentFrame = currentFrameLabel;
        buttonMode = true;
    }
    ...
    public function set data(value:Object):void {
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.grasscw(this, DebugUtils.set("Button.set data", _data, value, name));
        _data = value;
    }
    ...
    override public function set enabled(value:Boolean):void {
        ...
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2cw(this, DebugUtils.set("Button.enabled", super.enabled, value, name));
        super.enabled = value;
    ...
    public function set selected(value:Boolean):void {
        ...
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2cw(this, DebugUtils.set("set selected", _selected, value, name));
        _selected = value;
    ...
    public function set label(value:String):void {
        ...
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2cw(this, DebugUtils.set("Button.set label", _label, value, name));
        _label = value;
        invalidateData();
    }
    ...
    public function set autoSize(value:String):void {
        ...
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2cw(this, "set autoSize", name, _autoSize, ">", value, "label", _label);
        _autoSize = value;
        invalidateData();
    }
    ...
    override protected function draw():void {
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2cw(this, "Button.draw name:", name);
        // State is invalid, and has been set (is not the default)
        if (isInvalid(InvalidationType.STATE)) {
            if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.orangec(this, "Button.draw() - invalid state, _newFrame:", _newFrame);
            if (_newFrame) {
                if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.yellow2c(this, DebugUtils.set("Button.gotoAndPlay", _currentFrame, _newFrame));
                gotoAndPlay(_newFrame);
                if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.pinkc(this, DebugUtils.set("_currentFrame", _currentFrame, _newFrame), DebugUtils.set("_newFrame", _newFrame, null));
                _currentFrame = _newFrame;
                _newFrame = null;
            }
            
            if (_newFocusIndicatorFrame) {
                if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.orangec(this, DebugUtils.set("Button.focusIndicator.gotoAndPlay", focusIndicator.currentFrameLabel, _newFocusIndicatorFrame));
        ...
        if (isInvalid(InvalidationType.DATA)) {
            if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.orangec(this, "Button.draw() - invalid data");
            updateText();
            if (autoSize != TextFieldAutoSize.NONE) {
                if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2c(this, "autoSize != TextFieldAutoSize.NONE");
                invalidateSize();
            }
        }
        ...
        if (isInvalid(InvalidationType.SIZE)) {
            if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.orangec(this, "Button.draw() - invalid size");
            if (!preventAutosizing) {
                if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2c(this, "!preventAutosizing");
                alignForAutoSize();
                setActualSize(_width, _height);
            }
            if (!constraintsDisabled) {
                if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2c(this, "!constraintsDisabled");
    ...
    protected function handleMouseRollOver(event:MouseEvent):void {
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2cw(this, "handleMouseRollOver name:", name);
        ...
        if (event.buttonDown) {
            ...
            if (lockDragStateChange && Boolean(_mouseDown << mouseIdx & 1)) {
                return;
            }
    ...
    protected function handleMouseRollOut(event:MouseEvent):void {
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2cw(this, "handleMouseRollOut name:", name);
        var sfEvent:MouseEventEx = event as MouseEventEx;
        ...
        if (event.buttonDown) {
            ...
            if (lockDragStateChange || !enabled) {
                return;
            }
            if (_focused || _displayFocus) {
    ...
    protected function handleClick(controllerIndex:uint = 0):void {
        if (toggle && (!selected || allowDeselect)) {
            selected = !selected;
        }
    }
    ...
    protected function setState(state:String):void {
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.greencw(this, DebugUtils.set("setState", _state, state, name), "/ _currentFrame:", _currentFrame, "_newFrame:", _newFrame, "currentFrameLabel:", currentFrameLabel);
        _state = state;
        ...
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2c(this, "_labelHash:", _labelHash, "prefixes:", prefixes, "states:", states);
        var prefixesLength:uint = prefixes.length;
        for (var i:uint = 0; i < prefixesLength; i++) {
            var prefix:String = prefixes[i];
            var statesLength:uint = states.length;
            for (var j:uint = 0; j < statesLength; j++) {
                var newLabel:String = prefix + states[j];
                if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2c(this, "newLabel:", newLabel);
                if (_labelHash[newLabel]) {
                    if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2c(this, "has label:", newLabel, "_currentFrame:", _currentFrame, "currentFrameLabel:", currentFrameLabel);
    
                    if (newLabel == _currentFrame) {
                        _newFrame = null;
                        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.grass2c(this, "NOT set + RESET new ; _currentFrame:", _currentFrame, "_newFrame:", _newFrame);
                    } else {
                        if (newLabel == _newFrame) {
                            if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.grass2c(this, "NOT set IGNORE ; _currentFrame:", _currentFrame, "_newFrame:", _newFrame);
                        } else {
                            if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.grassc(this, "set _newFrame ; _currentFrame:", _currentFrame, "_newFrame:", _newFrame);
                            _newFrame = newLabel;
                            invalidateState();
                        }
                    }
    ...
    protected function getStatePrefixes():Vector.<String> {
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2cw(this, "Button.getStatePrefixes _selected:", _selected, "statesSelected:", statesSelected, "statesDefault:", statesDefault);
    ...
    protected function updateAfterStateChange():void {
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2c(this, "updateAfterStateChange initialized:", initialized);
        if (!initialized) {
            return;
        }
