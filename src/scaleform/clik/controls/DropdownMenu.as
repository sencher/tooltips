...
//        if (interesting) Cc.green2cw(this, "name:", name, DebugUtils.set("selectedIndex", _selectedIndex, value));
...
//        if (interesting) Cc.green2cw(this, "dataProvider", value);
...
//        if (interesting) Cc.green2cw(this, "showDropdown");
        if (dropdown == null) {
            return;
        }
        ...
        if (dd) {
            if (itemRenderer is String && itemRenderer != "") {
                dd.itemRenderer = domain.getDefinition(itemRenderer.toString()) as Class;
            } else if (itemRenderer is Class) {
                dd.itemRenderer = itemRenderer as Class;
            }
            
            if (scrollBar is String && scrollBar != "") {
                dd.scrollBar = domain.getDefinition(scrollBar.toString()) as Class;
            } else if (scrollBar is Class) {
                dd.scrollBar = scrollBar as Class;
            }
    ...
    protected function hideDropdown():void {
        if (_dropdownRef) {
            if (_dropdownRef.parent) _dropdownRef.parent.removeChild(_dropdownRef);
            _dropdownRef = null;
        }
    }
