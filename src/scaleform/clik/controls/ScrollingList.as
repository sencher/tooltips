...
public class ScrollingList extends CoreList {
    ...
    override public function set selectedIndex(value:int):void {
        ...
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2cw(this, DebugUtils.set("ScrollingList.set selectedIndex", _newSelectedIndex, value, name));
    ...
    public function get rowHeight():Number {
        return isNaN(_autoRowHeight) ? _rowHeight : _autoRowHeight;
    }
    ...
    override protected function updateSelectedIndex():void {
        ...
        if (_totalRenderers == 0) {
            return;
        } // Return if there are no renderers
    ...
    override protected function calculateRendererTotal(width:Number, height:Number):uint {
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.green2cw(this, "ScrollingList.calculateRendererTotal width:", width, "height:", height);
        if (isNaN(_rowHeight) && isNaN(_autoRowHeight)) {
            if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.greenc(this, "if (isNaN(_rowHeight) && isNaN(_autoRowHeight))");
            var renderer:IListItemRenderer = createRenderer(0);
            _autoRowHeight = renderer.height;
            cleanUpRenderer(renderer);
        }
    
        if (DebugAssistant.EXTENDED_INTERESTING && interesting) Ct.greenc(this, "return (availableHeight - padding.vertical) / rowHeight >> 0;", (availableHeight - padding.vertical) / rowHeight >> 0);
