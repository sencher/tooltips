package wowp.utils.data.filtering {
...
//        Cc.yellowcw(this, "addFilter filterID:", filterID, "_filters:", _filters.concat());
        if (!hasFilter(filterID)) {
            _filters[_filters.length] = filterID;
//            Cc.yellow("added", _filters.concat());
        }
...
//        Cc.redcw(this, "removeFilter", filterID, _filters.concat());
        var i:int = _filters.indexOf(filterID);
        if (i != -1) {
//            Cc.red("removed", _filters.concat());
            _filters.splice(i, 1);
        }
        invalidate();
    }
...
//            Cc.purple("criterials", _filters[i], filterFunction);
            if (filterFunction != null) {
                _filterFunctions.push(filterFunction);
            }
//            Cc.purple(_filterFunctions.length, _filterFunctions);
        }
        //	обратные фильтры
        for (var s:String in _revertedCreterias) {
//            Cc.purple("rev criterials", s, !hasFilter(s));
            if (!hasFilter(s)) {
...
//        Cc.blue("filteringTestFunction", item);
//			var isInteresting:Boolean = DebugUtils.isInterestingObj(item);
//        if (isInteresting) Cc.green2("filteringTestFunction", item, _filterFunctions.length);
        for each(var f:Function in _filterFunctions) {
//            if (isInteresting) Cc.green2(!f(item));
            if (!f(item)) {
//                if (isInteresting) Cc.green2c(this, item.planeID, false);
                return false;
            }
        }
//        if (isInteresting) Cc.green2c(this, item.planeID, true);
        return true;
    }
    
    public static function returnTrue(...args):Boolean {
        return true;
    }
}
