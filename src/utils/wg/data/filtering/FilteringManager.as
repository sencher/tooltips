package utils.wg.data.filtering
{
...
import wowp.utils.timeout.IValidator;
	...
	public class FilteringManager
	{
		...
		{
//			Cc.yellowcw(this, "addFilter", filterID, _filters.concat());
...
//				Cc.yellow("added", _filters.concat());
		...
		{
//			Cc.warncw(this, "removeFilter", filterID, _filters.concat());
			var i:int = _filters.indexOf(filterID);
			if (i != -1) {
//				Cc.warn("removed", _filters.concat());
				_filters.splice(i, 1);
			}
			invalidate();
		}
...
//				Cc.purplec(this, "criterials", _filters[i], filterFunction);
...
//				Cc.purplec(this, "rev criterials", s, !hasFilter(s));
				if (!hasFilter(s)) {
...
//			if(isInteresting) Cc.log("filteringTestFunction", item, _filterFunctions.length);
			for each(var f:Function in _filterFunctions) {
//				if(isInteresting) Cc.log(!f(item));
				if (!f(item)) {
//					if(isInteresting) Cc.logc(this, item.planeID, false);
					return false;
				}
			}
//			if(isInteresting) Cc.logc(this, item.planeID, true);
			return true;
		}
