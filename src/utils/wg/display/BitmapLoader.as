...
public class BitmapLoader extends Sprite {
    ...
    override public function get height():Number {
        if (mcSize) {
            //if(DebugUtils.isInterestingString(path)) Cc.log("mcSize.height", mcSize.height);
            return mcSize.height;
        }
        //if(DebugUtils.isInterestingString(path)) Cc.log("super.height", super.height);
        return super.height;
    }
...
class BitmapAutoLoader extends Bitmap implements ICacheClient {
    ...
    public function load(path:String, originalWidth:Number, originalHeight):void {
        //if(DebugUtils.isInterestingString(_path)) Cc.warncw(this, "load", path, originalWidth, originalHeight);
        ...
        if (path) {
            ...
            if (_path.indexOf("/") > -1 || _path.indexOf("\\") > -1) {
                //if(DebugUtils.isInterestingString(_path)) Cc.warn("Cache.getResource(_path, this);");
                Cache.getResource(_path, this);
            } else {
                //if(DebugUtils.isInterestingString(_path)) Cc.warn("onResourceLoaded(createObject(_path));");
    ...
    public function onResourceLoaded(resource:Object):void {
        //if(DebugUtils.isInterestingString(_path)) Cc.warncw(this, "onResourceLoaded", resource.height);
...
//        if(DebugUtils.isInterestingString(_path)) Cc.purplecw(this, "draw _isLoaded", _isLoaded, "_lockSize", _lockSize,
//                "_isHorizontallyCentered", _isHorizontallyCentered, "_isVerticallyCentered", _isVerticallyCentered);
        if (!_isLoaded) return;
        if (_lockSize) {
            scaleX = 1;
            scaleY = 1;
            ...
            //if(DebugUtils.isInterestingString(_path)) Cc.purple("height", h, _originalHeight, height);
            if (w > h) {
                //if(DebugUtils.isInterestingString(_path)) Cc.purple("w > h");
                scaleX = scaleY = h;
        ...
        //if(DebugUtils.isInterestingString(_path)) Cc.yellow("x",x,"y",y,"scaleX",scaleX);
    }
}
